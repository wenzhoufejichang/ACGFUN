/**
 * 弹幕播放器核心 Created by acgit on 2015/7/23. Copyright 2015 by Ruiko Of AcGit.cc
 * 
 * @license MIT
 * 
 * 版本2.0 2015/08/12
...
(function($) {
	...
	var DanmuPlayer = function(element, options) {
		...
		this.$element.append('<div class="danmu-div"id="'
				+ $(element).attr("id") + '-danmu-div" ></div>');
		...
		this.$element.append('<video class="danmu-video" src="' + options.src
		...
		this.$ctrlMain
				.append('<div class="play-btn ctrl-btn" style="background:#f36c60;"><span class="iplay" aria-hidden="true"></span></div>');
		this.$ctrlMain
				.append('<div class="current-time time-text ctrl-btn">0:00</div>');
		this.$ctrlMain.append('<div class="slash time-text ctrl-btn">/</div>');
		this.$ctrlMain
				.append('<div class="duration ctrl-btn time-text" >0:00</div>');
		this.$ctrlMain
				.append('<input class="danmu-input ctrl-btn form-control"   type="textarea" id="danmu_text" max=300 />'); // ->
		// button
		// あ
		this.$ctrlMain
				.append('<div class=" send-btn  ctrl-btn" style="background-color:#f36c60; border-radius:5px; margin-left:-50px;" >发送 ></div>');
		this.$ctrlMain
				.append('<div class="full-screen  ctrl-btn-right"><span class="" aria-hidden="true">全屏</span></div>');
		this.$ctrlMain
				.append('<div class="show-danmu  ctrl-btn-right ctrl-btn-right-active"><span class="" aria-hidden="true">弹幕off</span></div>');
		...
		this.sendDanmu = function(e) {
			...
			var textObj = '{ "text":"' + text + '","color":"' + color
			...
			// ==============将弹幕信息发送到服务器 [[==============
			/*
			 * var url = "${pageContext.request.contextPath}/index-danmu-ajax";
			 * var args = { "videoId" : videoId, "text" : text, "color" : color,
			 * "size" : size, "position" : position, "time" : time };
		...
		this.playPause = function(e) {
			if (e.data.video.paused) {
				...
				$(e.data.that.id + " .play-btn span").removeClass("iplay")
						.addClass("ipause");
			} else {
				e.data.video.pause();
				$(e.data.that.id + " .danmu-div").danmu('danmuPause');
				$(e.data.that.id + " .play-btn span").removeClass("ipause")
						.addClass("iplay");
			}
		};
		...
		var mainTimer = setInterval(function() {
			// 缓冲条
			var bufTime = $(that.id + " .danmu-video").get(0).buffered.end($(
					that.id + " .danmu-video").get(0).buffered.length - 1);

			var buffPrecent = (bufTime / that.duration) * 100;
			$(that.id + ".danmu-player .ctrl-progress .buffered ").css("width",
					buffPrecent + "%");
			// 时间轴修正
			// if (Math.abs($(that.id + " .danmu-div").data("nowTime") -
			// parseInt(that.video.currentTime)*10) > 1) {
			// $(that.id + " .danmu-div").data("nowTime",
			// parseInt(that.video.currentTime)*10);
		...
		var secTimer = setInterval(function() {
			// if (Math.abs($(that.id + " .danmu-div").data("nowTime") -
			// parseInt(that.video.currentTime*10)) > 1) {
			// console.log("revise time"+$(that.id + "
			// .danmu-div").data("nowTime")+ ","+that.video.currentTime*10);
			$(that.id + " .danmu-div").data("nowTime",
					parseInt(that.video.currentTime * 10));
		...
		$(this.id + " .danmu-video")
				.on(
						...
						function(e) {
							...
							$(e.data.that.id + " .danmu-player-load").css(
									"display", "block");
		...
		$(this.id + " .loop-btn").on(
				...
				function(e) {
					if (!e.data.that.isLoop) {
						...
						$(e.data.that.id + " .loop-btn").addClass(
								"ctrl-btn-right-active");
					} else {
						e.data.that.video.loop = true;
						e.data.that.isLoop = false;

						$(e.data.that.id + " .loop-btn").removeClass(
								"ctrl-btn-right-active");
					}
				});
		...
		$(this.id + " .danmu-video")
				.on(
						...
						function(e) {
							...
							var duraSec = parseInt(e.data.that.duration % 60) < 10 ? "0"
									+ parseInt(e.data.that.duration % 60)
									: parseInt(e.data.that.duration % 60);
							$(e.data.that.id + " .duration").text(
									duraMin + ":" + duraSec);
							$(e.data.that.id + " .danmu-video")
									.on(
											'timeupdate',
											{
												video : e.data.video,
											...
											function(e) {
												...
												var curSec = parseInt(current % 60) < 10 ? "0"
														+ parseInt(current % 60)
														: parseInt(current % 60);
												$(
														e.data.that.id
												...
												var duraSec = parseInt(e.data.that.duration % 60) < 10 ? "0"
														+ parseInt(e.data.that.duration % 60)
														: parseInt(e.data.that.duration % 60);
												$(e.data.that.id + " .duration")
														.text(
																duraMin
																		+ ":"
																		+ duraSec);
												var percentage = 100 * current
														/ e.data.that.duration;
												$(
														e.data.that.id
		...
		$(this.id + " .ctrl-progress")
				.on(
						...
						function(e) {
							var sumLen = $(e.data.that.id + " .ctrl-progress")
									.width();
							var pos = e.pageX
									- $(e.data.that.id + " .ctrl-progress")
											.offset().left;
	...
	function Plugin(option, arg) {
		return this.each(function() {
			var $this = $(this);
			var options = $.extend({}, DanmuPlayer.DEFAULTS,
					typeof option == 'object' && option);
			var data = $this.data('DanmuPlayer');
			var action = typeof option == 'string' ? option : NaN;
			if (!data)
				$this.data('danmu', (data = new DanmuPlayer(this, options)));
			if (action)
				data[action](arg);
		})
	}
