...
function Tip(msg) {
	$(".tishi").show().html(
			"<div class='prompt'><i class='tishi_icon'></i>" + msg + "</div>");
}
function Tips(msg) {
...
jQuery(function() {
	...
	$(".Program_name ").hover(function() {
		$(this).addClass("hover");
		$(this).find(".Program_title").stop().animate({
			opacity : 1,
			height : "100%"
	...
	$(".user_casestyle").hover(function() {
		...
		// $(this).stop().animate({bottom: "0px", opacity:1 , height:""},
		// "fast")
	}, function() {
		$(this).removeClass("hover");
	...
	$(".bannerNimg").css("background-image",
			"url(" + $(".bannerNimg").find("img").attr("src") + ")");
	// 置顶事件
	function topBack() {
		$('body,html').animate({
			scrollTop : 0
	...
	return $(this)
			.each(
					function() {
						...
						// how many times to update the value, and how much to
						// increment the value on each update
						...
						function render(value) {
							var formattedValue = settings.formatter.call(self,
									value, settings);
...
$.fn.countTo.defaults = {
	...
	speed : 300, // how long it should take to count between the target
	// numbers
